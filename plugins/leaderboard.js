let handler = async (m, { conn, args, participants }) => {
  ...
  let sortedATM = users.map(toNumber('bank')).sort(sort('bank'))
...
${sortedATM.slice(0, len).map(({ jid, bank }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *ATM ${bank}*`).join`\n`}
`.trim()
  conn.reply(m.chat, text, m, {
    contextInfo: {
      mentionedJid: [...usersExp.slice(0, len), ...usersLim.slice(0, len), ...usersLevel.slice(0, len), ...usersMoney.slice(0, len)], ...usersATM.slice(0, len)].filter(v => !participants.some(p => v === p.jid))
    }
  })
}
